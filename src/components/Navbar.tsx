...
const STEPS: { key: StepKey; label: string; number: number }[] = [
  ...
  { key: 'theme-select', label: 'Pilih Frame', number: 2 },
  { key: 'camera', label: 'Kamera', number: 3 },
  { key: 'filter', label: 'Filter Warna', number: 4 },
  { key: 'slotting', label: 'Tata ke Slot', number: 5 },
  { key: 'overlay', label: 'Overlay Frame', number: 6 },
  { key: 'preview-locked', label: 'Pratinjau', number: 7 },
  { key: 'checkout', label: 'Pembayaran', number: 8 },
];
...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const currentIdx = isFinalStep
    ? STEPS.length - 1
    : STEPS.findIndex(
        (s) => s.key === currentStep || (currentStep === 'landing' && s.key === 'event-info')
      );
  const currentStepObj = isFinalStep
    ? { key: 'final' as StepKey, label: 'Selesai', number: 8 }
          ...
          <div className="min-w-0">
            ...
            {!isStep1 && (
              <span className="text-[10px] text-amber-400 font-mono tracking-wider truncate block leading-tight">
                Step {currentStepObj.number}/8: {currentStepObj.label}
              </span>
            )}
          </div>
        </button>
