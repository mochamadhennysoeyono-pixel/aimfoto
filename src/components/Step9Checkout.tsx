...
export const Step9Checkout: React.FC<Step9CheckoutProps> = ({
  ...
  const [staticQris, setStaticQris] = useState<StaticQrisConfig | null>(null);
  const [isDownloadingQris, setIsDownloadingQris] = useState(false);
  const [frameBoomerangUrl, setFrameBoomerangUrl] = useState<string | null>(null);
  const isFrameBoomerangCompilingRef = useRef(false);
  ...
  // Otomatis compile video 1-Frame Utuh (gabungan semua slot foto diam + animasi boomerang dalam frame layout)
  useEffect(() => {
    if (!hasBoomerang || isFrameBoomerangCompilingRef.current || !layout) return;
    isFrameBoomerangCompilingRef.current = true;

    async function compileAndUploadFrameBoomerang() {
      try {
        ...
        const compiled = await compileCompositedAnimatedFrameVideo({
          layout,
          slotPhotos,
          capturedPhotos: session.capturedPhotos,
          slotBoomerangs,
          slotAdjustments: session.slotAdjustments,
          filter: filter || { id: 'normal', name: 'Normal', cssFilter: 'none' },
        ...
        if (compiled.videoBlob || compiled.videoUrl) {
          setFrameBoomerangUrl(compiled.videoUrl);
          const uploadRes = await uploadBoomerangToStorage(
            activeSessionId,
            compiled.videoBlob || compiled.videoUrl,
            'frame'
          );

          const finalPublicUrl = uploadRes.success && uploadRes.publicUrl ? uploadRes.publicUrl : compiled.videoUrl;
          ...
          try {
            ...
            cachedBoomerang[activeSessionId] = {
              ...
              frameVideoUrl: finalPublicUrl,
              primaryVideoUrl: finalPublicUrl,
        ...
        console.warn('Gagal compile 1-frame video boomerang di Checkout:', compileErr);
      }
    }

    compileAndUploadFrameBoomerang();
  }, [hasBoomerang, activeSessionId, layout, filter, frameUrl, session]);
  ...
  const saveSessionAndOrderToSupabase = async (
    ...
    try {
      ...
      if (hasBoomerang) {
        try {
          ...
          cachedBoomerang[activeSessionId] = {
            ...
            hasBoomerang: true,
            slots: activeBoomerangSlots,
            frameVideoUrl: frameBoomerangUrl || cachedBoomerang[activeSessionId]?.frameVideoUrl || '',
            primaryVideoUrl: frameBoomerangUrl || cachedBoomerang[activeSessionId]?.primaryVideoUrl || '',
            slotBoomerangs: session.slotBoomerangs || {},
            updatedAt: new Date().toISOString(),
          };
          localStorage.setItem('photobooth_boomerang_sessions', JSON.stringify(cachedBoomerang));
      ...
      // 4. Upload video 1-Frame Boomerang & klip slot ke Supabase Storage di background jika ada
      if (hasBoomerang) {
        // Upload 1-frame video utuh
        if (frameBoomerangUrl) {
          uploadBoomerangToStorage(activeSessionId, frameBoomerangUrl, 'frame').catch(console.warn);
        } else if (layout) {
          // Jika belum selesai render, compile & upload langsung
          const slotPhotos: Record<number, string> = { ...(session.slotAssignments || {}) };
          if (Object.keys(slotPhotos).length === 0 && session.capturedPhotos) {
            session.capturedPhotos.forEach((p, idx) => {
              slotPhotos[idx] = p;
          ...
          compileCompositedAnimatedFrameVideo({
            ...
            .then((res) => {
              if (res.videoBlob || res.videoUrl) {
                uploadBoomerangToStorage(activeSessionId, res.videoBlob || res.videoUrl, 'frame').then((uRes) => {
                  if (uRes.success && uRes.publicUrl) {
                    try {
                      const cached = JSON.parse(localStorage.getItem('photobooth_boomerang_sessions') || '{}');
                      ...
                      localStorage.setItem('photobooth_boomerang_sessions', JSON.stringify(cached));
                    } catch (e) {
            ...
            .catch((e) => console.warn('Gagal compile 1-frame video saat checkout:', e));
        }

        // Upload klip slot perorangan sebagai backup
  ...
  // Realtime Supabase Listener & Poller: Saat admin mengonfirmasi status transaksi menjadi 'success' di Admin Portal,
  // photobooth pelanggan akan otomatis berpindah ke Step 10 (Selesai / Cetak & Download)
  useEffect(() => {
    if (!activeSessionId) return;

    const channel = supabase
      .channel(`order-status-update-${activeSessionId}`)
      .on(
        'postgres_changes',
        {
          event: 'UPDATE',
        ...
        (payload: any) => {
          ...
          if (
            newRecord &&
            (newRecord.payment_status === 'success' ||
              newRecord.payment_status === 'settlement' ||
              newRecord.status === 'success' ||
              newRecord.status === 'paid')
          ) {
            triggerSuccessTransition(newRecord.payment_method || 'Konfirmasi Kasir/Admin', newRecord.id);
          }
        }
      )
      .subscribe();

    // Polling backup setiap 3 detik untuk memastikan transisi responsif jika websocket ada jeda
    const poller = setInterval(async () => {
      try {
        ...
        if (
          ...
          triggerSuccessTransition(data.payment_method || 'Konfirmasi Kasir/Admin', data.id);
        }
      } catch (_) {}
    }, 3000);
  ...
  // Handler: Pembayaran QRIS - Buka WA Konfirmasi (Tetap di Step 8, tidak ke Step 10)
  ...
  if (isFreeEvent) {
    return (
      <div className="flex-1 flex flex-col justify-between max-w-md mx-auto w-full p-4 overflow-y-auto space-y-4 pb-36 sm:pb-24">
  ...
  return (
    <div className="flex-1 flex flex-col justify-between max-w-lg mx-auto w-full p-4 overflow-y-auto space-y-3 pb-36 sm:pb-24">
      {/* Header Langkah */}
      ...
      {selectedMethod === 'cash' && (
        <div className="p-4 rounded-2xl bg-zinc-950 border border-zinc-800/80 space-y-3.5 shadow-xl">
          ...
          <div className="bg-zinc-900/90 rounded-xl p-3 border border-zinc-800 space-y-2.5 text-xs">
            ...
            <div className="space-y-2 text-[11px] text-zinc-300">
              <div className="flex items-start gap-2">
                ...
                <span>
                  Klik tombol <strong>Konfirmasi ke WA Admin</strong> di bawah untuk mengirim data sesi <span className="font-mono text-emerald-300">{shortTicketId}</span>.
                </span>
              </div>
              <div className="flex items-start gap-2">
                <span className="w-4 h-4 rounded-full bg-emerald-500/20 text-emerald-400 font-bold flex items-center justify-center text-[10px] shrink-0 mt-0.5">
              ...
              <div className="flex items-start gap-2 p-2.5 rounded-lg bg-emerald-500/10 border border-emerald-500/25 text-emerald-300">
                <Sparkles className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" />
                <span className="leading-snug text-[11px]">
                  <strong>Catatan:</strong> Jika sudah melakukan konfirmasi via WhatsApp dan pembayaran tunai diterima oleh kasir/admin, file foto &amp; video Anda akan segera dikirimkan langsung ke nomor WhatsApp Anda!
                </span>
              </div>
            </div>
          </div>
          ...
          {/* Tombol Aksi Tunai: Hanya Tombol Konfirmasi WA */}
          <div className="space-y-2.5 pt-1">
            <button
              type="button"
              onClick={handleOpenWhatsapp}
              disabled={isProcessing}
              className="w-full py-3.5 px-4 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs sm:text-sm shadow-lg shadow-emerald-600/25 flex items-center justify-center gap-2 transition-all cursor-pointer disabled:opacity-50"
            >
              {isProcessing ? (
                <>
                  <Loader2 className="w-4 h-4 animate-spin" />
                  <span>Menyimpan & Menghubungkan ke WA...</span>
                </>
              ) : (
                <>
                  <MessageSquare className="w-4 h-4" />
                  <span>Konfirmasi ke WA Admin (+{targetWaNumber})</span>
                </>
              )}
            </button>

            {isSubmitted && (
              <div className="p-3.5 rounded-xl bg-emerald-950/40 border border-emerald-500/30 text-center space-y-2 animate-in fade-in">
                <div className="flex items-center justify-center gap-1.5 text-emerald-400 font-semibold text-xs">
                  <CheckCircle2 className="w-4 h-4" />
                  <span>Konfirmasi WhatsApp Berhasil Dibuka</span>
                </div>
                <p className="text-[11px] text-zinc-300 leading-relaxed">
                  Silakan serahkan uang tunai ke kasir. Admin akan mengecek dan memproses pesanan Anda secara manual, lalu mengirimkan file ke nomor WhatsApp Anda.
                </p>
                {onRestart && (
                  <button
                    type="button"
                    onClick={onRestart}
                    className="mt-1 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-zinc-200 text-xs font-medium cursor-pointer transition-colors border border-zinc-700"
                  >
                    <RotateCcw className="w-3.5 h-3.5 text-amber-400" />
                    <span>Selesai &amp; Mulai Sesi Baru</span>
                  </button>
                )}
              </div>
            )}
      ...
      {selectedMethod === 'qris' && (
        <div className="p-4 rounded-2xl bg-zinc-950 border border-zinc-800/80 space-y-3.5 shadow-xl text-center">
          ...
          <div className="bg-zinc-900/90 rounded-xl p-3 border border-zinc-800 space-y-2 text-left text-xs">
            ...
            <div className="space-y-1.5 text-[11px] text-zinc-300">
              ...
              <div className="flex items-start gap-2">
                ...
                <span>
                  Klik tombol <strong>Chat WhatsApp Admin</strong> di bawah untuk mengirimkan bukti bayar.
                </span>
              </div>
              <div className="flex items-start gap-2">
                <span className="w-4 h-4 rounded-full bg-amber-500/20 text-amber-400 font-bold flex items-center justify-center text-[10px] shrink-0 mt-0.5">
                ...
                <span>
                  Admin akan memverifikasi bukti secara manual, {isPrint ? 'mencetak foto fisik, dan ' : ''}mengirimkan soft file ke WhatsApp Anda!
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col items-center justify-center py-1">
            <div className="p-3 bg-white rounded-2xl shadow-xl border-2 border-zinc-200 inline-block w-52 h-52 flex items-center justify-center overflow-hidden">
              {staticQris?.url ? (
                <img
                  src={staticQris.url}
                  alt="QRIS Statis"
                  ...
                  onError={() => {
                    console.warn('QRIS custom image load failed, reverting to QR generator fallback');
                    setStaticQris((prev) => (prev ? { ...prev, url: null } : null));
                  }}
                />
              ) : qrisQrCode ? (
                <img src={qrisQrCode} alt="QRIS Code" className="w-full h-full object-contain" />
              ) : (
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  await downloadQrisImage(
                    staticQris?.url || qrisQrCode,
                    staticQris?.merchantName || eventConfig.nama
                  );
                } finally {
              ...
              className="mt-2 inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-zinc-900 hover:bg-zinc-800 border border-zinc-700 text-zinc-200 text-xs font-semibold cursor-pointer transition-colors shadow-sm disabled:opacity-50"
            >
              {isDownloadingQris ? (
                <Loader2 className="w-3.5 h-3.5 animate-spin" />
              ) : (
                <Download className="w-3.5 h-3.5 text-amber-400 stroke-[2.2]" />
              )}
              <span>Download Barcode QRIS ke HP</span>
            </button>
          </div>

          {/* Tombol Aksi QRIS: Buka Chat WA & Tetap di Step 8 */}
          <div className="space-y-2.5 pt-1">
            <button
              type="button"
              onClick={handleConfirmQrisPaid}
              disabled={isProcessing}
              className="w-full py-3.5 px-4 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs sm:text-sm shadow-xl shadow-emerald-600/25 flex items-center justify-center gap-2 transition-all cursor-pointer disabled:opacity-50"
            >
              {isProcessing ? (
                <>
                  <Loader2 className="w-4 h-4 animate-spin" />
                ...
                <>
                  <MessageSquare className="w-4 h-4" />
                  <span>Chat WhatsApp Admin (+{targetWaNumber})</span>
                </>
              )}
            </button>
            ...
            {isSubmitted && (
              <div className="p-3.5 rounded-xl bg-emerald-950/40 border border-emerald-500/30 text-center space-y-2 animate-in fade-in">
                ...
                <p className="text-[11px] text-zinc-300 leading-relaxed">
                  Silakan kirimkan bukti transfer / pembayaran QRIS Anda ke WhatsApp admin. Admin akan mengecek dan memverifikasi pembayaran secara manual, lalu mengirimkan file foto &amp; video Anda.
                </p>
                {onRestart && (
                  <button
                    type="button"
