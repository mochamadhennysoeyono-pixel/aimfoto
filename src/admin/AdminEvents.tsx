...
let cachedAdminEvents: AdminEventItem[] = [];

export const AdminEvents: React.FC<AdminEventsProps> = ({
  onSelectEventForFrames,
  onOpenKioskEvent,
}) => {
  const [events, setEvents] = useState<AdminEventItem[]>(cachedAdminEvents);
  const [isLoading, setIsLoading] = useState(cachedAdminEvents.length === 0);
