...
export function generatePrintHtml(imageUrl: string, options?: { title?: string; orderShort?: string }): string {
  ...
  const origin = typeof window !== 'undefined' ? window.location.origin : 'https://photobooth.aimspace.my.id';
  const absoluteImageUrl = (imageUrl.startsWith('http://') || imageUrl.startsWith('https://') || imageUrl.startsWith('data:') || imageUrl.startsWith('blob:'))
    ? imageUrl
...
<body>
  ...
  <div class="top-toolbar no-print">
    ...
    <div class="toolbar-actions">
      <a href="${absoluteImageUrl}${absoluteImageUrl.includes('?') ? '&' : '?'}download=1&filename=photobooth-${orderShort}.jpg" download="photobooth-${orderShort}.jpg" class="btn-secondary">
        ⬇️ Unduh File .JPG
      </a>
  ...
  <div class="preview-stage">
    <div class="photo-sheet">
      <img id="printTarget" src="${absoluteImageUrl}" alt="Foto Photobooth #${orderShort}" crossorigin="anonymous" onerror="handlePhotoError(this)" />
    </div>
  </div>

  <script>
    function handlePhotoError(el) {
      console.warn('Gagal memuat foto print dari URL utama:', el.src);
      if (el.src && el.src.indexOf('/api/r2/file/') !== -1) {
        var rawKey = el.src.split('/api/r2/file/')[1];
        if (rawKey) {
          el.onerror = function() {
            console.error('Fallback R2 direct gagal dimuat');
          };
          el.src = 'https://pub-9ab796572b1a43ad87628fe9260ddf61.r2.dev/' + rawKey.split('?')[0];
        }
      }
    }
...
export function openPrintTab(imageUrl: string, options?: { title?: string; orderShort?: string }): PrintResult {
  ...
  // Buka jendela baru secara langsung dan injeksikan HTML pada origin yang sama
  try {
    ...
    console.warn('Direct window.document.write failed, fallback to blob:', e);
  }

  // Fallback jika direct window.open dibatasi browser
  if (!openedWindow || openedWindow.closed || typeof openedWindow.closed === 'undefined') {
    try {
      openedWindow = window.open(blobUrl, '_blank');
    } catch (e2) {
      console.warn('window.open blob fallback failed:', e2);
    }
  }
...
export async function downloadPhotoFile(url: string, filename: string) {
  try {
    ...
    const absoluteUrl = (url.startsWith('http://') || url.startsWith('https://') || url.startsWith('data:') || url.startsWith('blob:'))
      ? url
      : `${origin}${url.startsWith('/') ? '' : '/'}${url}`;

    // Jika mengarah ke API R2, gunakan parameter download langsung dari worker
    if (absoluteUrl.includes('/api/r2/file/')) {
      const sep = absoluteUrl.includes('?') ? '&' : '?';
      const dlLink = `${absoluteUrl}${sep}download=1&filename=${encodeURIComponent(filename)}`;
