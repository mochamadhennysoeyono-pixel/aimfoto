...
import { AdminQris } from './AdminQris';
import { QrCode } from 'lucide-react';

export type AdminTab = 'events' | 'frames' | 'orders' | 'setup' | 'payment' | 'qris';
...
const navItems: NavItem[] = [
  ...
  { tab: 'qris', label: 'Upload QRIS Statis', shortLabel: 'QRIS', icon: QrCode },
  { tab: 'frames', label: 'Kelola Frames & Slot', shortLabel: 'Frames', icon: ImageIcon },
...
export const AdminPortal: React.FC<AdminPortalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<AdminTab>(() => {
    ...
    if (
      ...
      return 'payment';
    if (path.includes('/admin/qris')) return 'qris';
    if (path.includes('/admin/layouts') || path.includes('/admin/frames')) return 'frames';
  ...
  const navigateToTab = (tab: AdminTab) => {
    ...
    try {
      ...
      window.history.pushState({}, '', `${cleanBase}/admin/${tab}`);
    } catch (e) {
  ...
  useEffect(() => {
    const handlePopState = () => {
      const path = window.location.pathname.toLowerCase();
      if (path.includes('/admin/layouts') || path.includes('/admin/frames')) setActiveTab('frames');
