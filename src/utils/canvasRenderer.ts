    ...
    for (let sIdx = 0; sIdx < layout.slots.length; sIdx++) {
      ...
      if (currentPhotoSrc) {
        try {
          ...
          const naturalW = photoImg.naturalWidth || photoImg.width || 1200;
          const naturalH = photoImg.naturalHeight || photoImg.height || 1200;
          const imgAspect = (naturalW && naturalH) ? naturalW / naturalH : 1.0;
          const canvasAspect = width / height;

          // Hitung ukuran pixel foto proporsional sesuai rasio foto/viewfinder kamera (anti-crop)
          ...
          const baseH = adj?.height !== undefined ? adj.height : (baseW * canvasAspect) / imgAspect;
          ...
          ctx.save();
          ctx.beginPath();
          // Foto dibuat lancip tanpa border radius sesuai permintaan user
